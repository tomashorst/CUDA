...
int main(int argc, char **argv)
{
	...
	thrust::device_vector<unsigned long long> a(N);
	...
	for(int i=0; i < N; i++) sumCheck += i; //aunque todos sabemos que es N(N-1)/2

	// task 3: calculate the sum of the array
	unsigned long long sumA= thrust::reduce(a.begin(),a.end(), (unsigned long long)0, thrust::plus<unsigned long long>());
