...
struct Convolutor
{
	__device__
  	cufftComplex operator()(cufftComplex c, unsigned int tid)
  	{
		...
		#ifdef LAPLACIAN 
		fac=-(2.0-2.0*cosf(qx))-(2.0-2.0*cosf(qy)); // FFT(laplaciano discreto)
		//fac=-(2.0-2.0*cosf(kx*2.0*M_PI/NX))-(2.0-2.0*cosf(ky*2.0*M_PI/NY)); // FFT(laplaciano discreto)
		#elif defined(FILTER) 
		float qcut = (2.0*M_PI/30.0); 
		//fac=(qx*qx+qy*qy<qcut*qcut)?(1.0):(0.0);
		//fac=(qy*qy<qcut*qcut)?(1.0):(0.0);
		//fac=1.0;

		//filtro gausiano
		fac=exp(-(qx*qx+qy*qy)*0.5/(qcut*qcut));
...
void llenar_array(thrust::device_vector<float> &d, unsigned long semi)
{
	...
	#elif defined(CUADRADO)
		thrust::fill(d.begin(),d.end(),0.0);
		float lado=NX/8;
		for(int i=NX/2-lado;i<=NX/2+lado;i++){
		for(int j=NY/2-lado;j<=NY/2+lado;j++){
			d[j+NY*i]=1.0;
		}}
	#else
...
int main()
{
	...
	// Si i=0,...,NX-1; j=0,...,NY-1
	// Vamos a acceder asi en el espacio real: v(x,y)=v[y+NY*x]
	// y en el espacio de Fourier?: TODO
	...
	int m[NRANK] = {NX, NY}; 
	if (cufftPlanMany(&planinverso, NRANK, m, NULL, 1, 0, NULL, 1, 0, CUFFT_C2R,BATCH) != CUFFT_SUCCESS)
	{ 
